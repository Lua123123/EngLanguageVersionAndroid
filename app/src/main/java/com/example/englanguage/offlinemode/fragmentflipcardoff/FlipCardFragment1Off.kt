...
import com.example.englanguage.databinding.FragmentFlipCard1OffBinding
import com.example.englanguage.model.vocabulary.SuccessVocabulary
import java.util.*

class FlipCardFragment1Off() : Fragment() {
    private lateinit var frontAnim: AnimatorSet
    private lateinit var behindAnim: AnimatorSet
    private var isFront = true
    private lateinit var binding: FragmentFlipCard1OffBinding
    ...
    override fun onCreateView(
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_flip_card1_off, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (mListVocabulary.isNotEmpty()) {
            ...
            binding.mButtonSpeak.setOnClickListener {
                val text: String = mListVocabulary[0].word
                speak(text)
            }
            binding.mButtonSpeak1.setOnClickListener {
                val text1: String = mListVocabulary[1].word
                speak(text1)
            }
            binding.mButtonSpeak2.setOnClickListener {
                val text2: String = mListVocabulary[2].word
                speak(text2)
            }
            binding.mButtonSpeak3.setOnClickListener {
                val text3: String = mListVocabulary[3].word
                speak(text3)
            }
            binding.mButtonSpeak4.setOnClickListener {
                val text4: String = mListVocabulary[4].word
                speak(text4)
            }
            binding.mButtonSpeak5.setOnClickListener {
                val text5: String = mListVocabulary[5].word
                speak(text5)
            }

            binding.cartFront.text = mListVocabulary[0].word
            binding.cartBehind.text = mListVocabulary[0].mean
            binding.cartFront1.text = mListVocabulary[1].word
            binding.cartBehind1.text = mListVocabulary[1].mean
            binding.cartFront2.text = mListVocabulary[2].word
            binding.cartBehind2.text = mListVocabulary[2].mean
            binding.cartFront3.text = mListVocabulary[3].word
            binding.cartBehind3.text = mListVocabulary[3].mean
            binding.cartFront4.text = mListVocabulary[4].word
            binding.cartBehind4.text = mListVocabulary[4].mean
            binding.cartFront5.text = mListVocabulary[5].word
            binding.cartBehind5.text = mListVocabulary[5].mean
        }
    }
