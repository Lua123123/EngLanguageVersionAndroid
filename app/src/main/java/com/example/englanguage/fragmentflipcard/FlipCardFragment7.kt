...
import com.example.englanguage.databinding.FragmentFlipCard7Binding
import com.example.englanguage.viewmodel.FlipViewModel
import org.koin.androidx.viewmodel.ext.android.viewModel
import java.util.*

class FlipCardFragment7 : Fragment() {
    private lateinit var frontAnim: AnimatorSet
    private lateinit var behindAnim: AnimatorSet
    private var isFront = true
    private lateinit var binding: FragmentFlipCard7Binding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_flip_card7, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.mButtonSpeak.setOnClickListener {
            val text: String? = flipViewModel.mVocabulary.value?.success?.get(36)?.word
            speak(text!!) }
        binding.mButtonSpeak1.setOnClickListener {
            val text1: String? = flipViewModel.mVocabulary.value?.success?.get(37)?.word
            speak(text1!!) }
        binding.mButtonSpeak2.setOnClickListener {
            val text2: String? = flipViewModel.mVocabulary.value?.success?.get(38)?.word
            speak(text2!!) }
        binding.mButtonSpeak3.setOnClickListener {
            val text3: String? = flipViewModel.mVocabulary.value?.success?.get(39)?.word
            speak(text3!!) }
        binding.mButtonSpeak4.setOnClickListener {
            val text4: String? = flipViewModel.mVocabulary.value?.success?.get(40)?.word
            speak(text4!!) }
        binding.mButtonSpeak5.setOnClickListener {
            val text5: String? = flipViewModel.mVocabulary.value?.success?.get(41)?.word
            speak(text5!!) }
        flipViewModel.mutableLiveDataClickGetVocabulary()

        flipViewModel.mVocabulary.observe(this)
        {
            binding.cartFront.text = it!!.success?.get(36)?.word
            binding.cartBehind.text = it.success?.get(36)?.mean
            binding.cartFront1.text = it.success?.get(37)?.word
            binding.cartBehind1.text = it.success?.get(37)?.mean
            binding.cartFront2.text = it.success?.get(38)?.word
            binding.cartBehind2.text = it.success?.get(38)?.mean
            binding.cartFront3.text = it.success?.get(39)?.word
            binding.cartBehind3.text = it.success?.get(39)?.mean
            binding.cartFront4.text = it.success?.get(40)?.word
            binding.cartBehind4.text = it.success?.get(40)?.mean
            binding.cartFront5.text = it.success?.get(41)?.word
            binding.cartBehind5.text = it.success?.get(41)?.mean
        }
    }
